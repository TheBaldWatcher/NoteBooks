...
do_build()
{
    ...
    # 编译安装目录
    ARCH_BUILD_DIR=$work_dir/build/android_static_noJni/$BUILD_ARCH
    ...
    cmake -DCMAKE_ANDROID_NDK_TOOLCHAIN_VERSION="$CMAKE_ANDROID_NDK_TOOLCHAIN_VERSION" \
          ...
          -DINCLUDE_JNI_CODE="NO"                             \
          -DINCLUDE_ASSET_CODE="NO"                             \
          -DMEITU_PHONE="NO"                             \
          $work_dir/
    #cmake --build .
    make all -j8
    ...
    if [ "$BUILD_TYPE" == "Release" ]; then
        NOSYMBOL_DIR=${work_dir}/libs_without_symbol/android_static_noJni/${BUILD_ARCH}
        SYMBOL_DIR=${work_dir}/libs_with_symbol/android_static_noJni/${BUILD_ARCH}/
        ...
        cp ${ARCH_BUILD_DIR}/*/*.a ${SYMBOL_DIR}
    fi
    popd # $ARCH_BUILD_DIR
}
